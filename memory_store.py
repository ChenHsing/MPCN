...
class MemoryStore(object):
    def __init__(self, memory_size, n_neighbours, embed_dim,label_dim, device):
        ...
        self.memory_labels = torch.ones((self.memory_size, self.label_dim, self.label_dim,
                                                          self.label_dim), dtype=torch.long).to(self.device)
        self.memory_iou = torch.ones(
            (self.memory_size,1)).to(device)
    ...
    def add_entry(self, embed, label):
        ...
        Returns:
            None
        """
        label = label.squeeze().__ge__(0.5).long()
    ...
    def get_nearest_entries(self, queries):
        ...
        mask_idx = min(self.memory_size, max(
            self.n_neighbours*2, self.added_memories))
        ...
        distances, idx = torch.topk(-dist, dim=1, k=self.n_neighbours*2)
...
if __name__ == '__main__':
    ...
    memory = MemoryStore(memory_size=4000,n_classes=13,n_neighbours=5,embed_dim=2048,label_dim=32,device=device)